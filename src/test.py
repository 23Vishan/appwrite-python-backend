...
def reduce_file_size():
    ...
    for zip_file in os.listdir(base_path):
        ...
        input_zip_path = os.path.join(base_path, zip_file)
        output_zip_path = os.path.join(filtered_path, zip_file)

        with zipfile.ZipFile(input_zip_path, 'r') as zf_in, \
             zipfile.ZipFile(output_zip_path, 'w') as zf_out:  # open once in write mode

            for file in zf_in.namelist():
                if file == f"{date}/":
                    continue  # skip directory entry

                with zf_in.open(file) as cf:
                    with gzip.GzipFile(fileobj=cf) as gz:
                        data = np.frombuffer(gz.read(), dtype=[('time', 'i4'), ('mid', 'f4')])
                        times = data['time']
                        mids = data['mid']
                        ...
                        zf_out.writestr(file, buffer.getvalue())
...
print(veic(100000000, 30, 1.5, 3, 1.1, 1, 2.0))
