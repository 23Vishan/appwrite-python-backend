...
def veic(timestamp_of_entry, spread_width, entry_credit, num_spreads, stop_price, limit_price, sl_mult):
    ...
    for zip_file in os.listdir(f"{BASE_PATH}"):
        # extract date
        ...
        dates.append(date)
    return round(total_profit, 2), daily_profits, dates
...
def main(context):
    try:
        ...
        # call veic
        total_profit, daily_profits, dates = veic(int(entry_time), int(spread_width), float(entry_credit), int(number_of_spreads), float(stop_price), float(limit_price), float(stop_loss_multiplier))
